...
function createOverlayWindow() {
  ...
  overlayWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: true,
      preload: path.join(__dirname, 'preload.js'),
      // Cache-Optimierungen für Overlay
      cache: false,
      webSecurity: true,
      offscreen: false
    }
...
app.whenReady().then(() => {
  ...
  // Globale Hotkeys registrieren (optional für normale Version)
  try {
    ...
    console.log('Hotkeys could not be registered (might be in use):', error.message);
  }
});
...
ipcMain.handle('game-speak', async (event, text, voiceSettings) => {
  if (overlayWindow) {
